...
TEST(SetFromList, Copy) {
    ...
    std::unique_ptr<IGraph> Sgraph = std::make_unique<SetGraph>(*Lgraph);
...
TEST(SetFromMatrix, Copy) {
    ...
    std::unique_ptr<IGraph> Sgraph = std::make_unique<SetGraph>(*Mgraph);
...
TEST(SetFromArc, Copy) {
    ...
    std::unique_ptr<IGraph> Sgraph = std::make_unique<SetGraph>(*Agraph);
...
TEST(SetFromSet, Copy) {
    ...
    std::unique_ptr<IGraph> newSgraph = std::make_unique<SetGraph>(*Sgraph);

    for (int i = 0; i < newSgraph->VerticesCount(); ++i) {
        auto tmp = newSgraph->GetNextVertices(i);
        std::sort(tmp.begin(), tmp.end());

        EXPECT_TRUE(next_cases[i] == tmp);

        tmp = newSgraph->GetPrevVertices(i);
        std::sort(tmp.begin(), tmp.end());
